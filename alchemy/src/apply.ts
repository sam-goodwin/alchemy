...
export async function apply<Out extends Resource>(
  ...
  try {
    async function apply(): Promise<Awaited<Out>> {
      await scope.init();
      return await _apply(resource, props, start, options);
    }
    if (scope.lock) {
      return await scope.withLock(resource[ResourceFQN], apply);
    } else {
      return await apply();
    }
  } catch (error) {
...
async function _apply<Out extends Resource>(
  ...
  try {
    output = await alchemy.run(
      ...
      async () => await provider.handler.bind(ctx)(resource[ResourceID], props),
    );
  } catch (error) {
