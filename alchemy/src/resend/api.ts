...
/**
 * Options for Resend API requests
 */
export interface ResendApiOptions {
  /**
  ...
  /**
   * API Key to use (overrides RESEND_API_KEY env var)
   */
  apiKey?: Secret;
}

/**
 * Create a ResendApi instance with environment variable fallback
 * @param options API options
 * @returns ResendApi instance
 */
export function createResendApi(
  options: Partial<ResendApiOptions> = {},
): ResendApi {
...
/**
 * Get authentication headers for Resend API
 * @param options ResendApiOptions
 * @returns Headers for authentication
 */
export async function getResendAuthHeaders(
  options: Partial<ResendApiOptions>,
): Promise<Record<string, string>> {
  const apiKey = options.apiKey?.unencrypted ?? process.env.RESEND_API_KEY;
...
/**
 * Resend API client using raw fetch
 */
export class ResendApi {
   ...
   * Use createResendApi factory function instead of direct constructor
   *
   * @param options API options
   */
  constructor(private readonly options: ResendApiOptions) {
    this.baseUrl = options.baseUrl ?? "https://api.resend.com";
  }

  /**
   * Make a fetch request to the Resend API
   *
  ...
  async fetch(path: string, init: RequestInit = {}): Promise<Response> {
    ...
    headers = {
      ...(await getResendAuthHeaders(this.options)),
      ...headers,
    };
